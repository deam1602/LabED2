...
namespace Laboratorio_ED2_1
{
    public class AVL<T> where T : IComparable<T>
    {
        ...
        public Nodo<T> GetByDPI(long dpi)
        {
            return GetByDPI(Raiz, dpi);
        }

        private Nodo<T> GetByDPI(Nodo<T> nodo, long dpi)
        {
            if (nodo == null)
            {
                return null;
            ...
            long nodoDPI = ((dynamic)nodo.elvalor).DPI;
            ...
            {
                return GetByDPI(nodo.Nodoizquierdo, dpi);
            }
            else
            {
                return GetByDPI(nodo.Nododerecho, dpi);
            }
        }
        ...
        {
            Nodo<T> nodo = Get(Raiz, persona); // Buscar el nodo con el valor DPI a actualizar
            if (nodo != null)
            {
            ...
            {
                return false; // No se encontró el valor DPI, no se pudo actualizar
            }
        }
    }
}
